...
class SRGAN(BaseEditModel):
    ...
    def __init__(
        ...
        # discriminator
        self.discriminator = discriminator if discriminator else None

        # loss
        self.gan_loss = gan_loss if gan_loss else None
        self.perceptual_loss = perceptual_loss if perceptual_loss else None

        self.disc_steps = (
            1 if self.train_cfg is None else self.train_cfg.get("disc_steps", 1)
        )
        self.disc_repeat = (
            1 if self.train_cfg is None else self.train_cfg.get("disc_repeat", 1)
        )
    ...
    def train_step(
        self, data: List[dict], optim_wrapper: OptimWrapperDict
