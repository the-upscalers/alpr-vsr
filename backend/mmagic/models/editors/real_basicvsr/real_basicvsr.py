...
from ..real_esrgan.real_esrgan import RealESRGAN
...
class RealBasicVSR(RealESRGAN):
    ...
    def __init__(
        ...
        self.cleaning_loss = cleaning_loss if cleaning_loss else None
    ...
    def extract_gt_data(self, data_samples):
        ...
        if self.cleaning_loss:
            ...
            gt_clean = F.interpolate(
                gt_clean, scale_factor=0.25, mode="area", recompute_scale_factor=False
            )
    ...
    def g_step(self, batch_outputs, batch_gt_data):
        ...
        losses = super().g_step(
            batch_outputs=fake_g_output, batch_gt_data=(gt_pixel, gt_percep, gt_gan)
        )
