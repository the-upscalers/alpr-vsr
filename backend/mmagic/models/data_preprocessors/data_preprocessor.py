...
class DataPreprocessor(ImgDataPreprocessor):
    ...
    def __init__(
        ...
        # get channel order
        assert output_channel_order is None or output_channel_order in ["RGB", "BGR"], (
    ...
    def _parse_channel_order(
        self, key: str, inputs: Tensor, data_sample: Optional[DataSample] = None
    ) -> str:
        ...
        channel_order_key = (
            "gt_channel_order" if key == "gt_img" else f"{key}_channel_order"
        )
        color_type_key = "gt_color_type" if key == "gt_img" else f"{key}_color_type"
    ...
    def _parse_batch_channel_order(
        self, key: str, inputs: Sequence, data_samples: Optional[Sequence[DataSample]]
    ) -> str:
        ...
        assert all([inputs_order == order for order in batch_inputs_orders]), (
    ...
    def _do_conversion(
        ...
        """
        if target_order is None or inputs_order.upper() == target_order.upper():
        ...
        def conversion(inputs, channel_index):
            ...
            inputs = torch.index_select(
                inputs, channel_index, torch.LongTensor(new_index).to(inputs.device)
            )
            return inputs
    ...
    def _do_norm(self, inputs: Tensor, do_norm: Optional[bool] = None) -> Tensor:
    ...
    def _preprocess_image_tensor(
        ...
        pad_w = target_w - w
        batch_inputs = F.pad(
            inputs, (0, pad_w, 0, pad_h), self.pad_mode, self.pad_value
        )

        padding_size = torch.FloatTensor((0, pad_h, pad_w))[None, ...]
        padding_size = padding_size.repeat(inputs.shape[0], 1)
        data_samples = self._update_metainfo(
            padding_size, {key: output_channel_order}, data_samples
        )
        return batch_inputs, data_samples

    def _preprocess_image_list(
        self,
        ...
        channel_order = self._parse_batch_channel_order(key, tensor_list, data_samples)
        dim = tensor_list[0].dim()
        assert all([tensor.ndim == dim for tensor in tensor_list]), (
        ...
        max_sizes = (
            torch.ceil(torch.max(all_sizes, dim=0)[0] / self.pad_size_divisor)
        ...
        if padding_sizes.sum() == 0:
            ...
            data_samples = self._update_metainfo(
                padding_sizes, {key: output_channel_order}, data_samples
            )
        ...
        for idx, tensor in enumerate(tensor_list):
            paded_tensor = F.pad(
                tensor, tuple(pad[idx].tolist()), self.pad_mode, self.pad_value
            )
    ...
    def _preprocess_dict_inputs(
        self, batch_inputs: dict, data_samples: Optional[SampleList] = None
        ...
        for k, inputs in batch_inputs.items():
            ...
            if isinstance(inputs, list):
                if k in self._NON_CONCATENATE_KEYS:
                    # use the first value
                    assert all([inputs[0] == inp for inp in inputs]), (
                        f"NON_CONCENTATE_KEY '{k}' should be consistency "
                        "among the data list."
                    )
                    batch_inputs[k] = inputs[0]
                else:
                    assert all([isinstance(inp, torch.Tensor) for inp in inputs]), (
                ...
                batch_inputs[k], data_samples = self._preprocess_image_tensor(
                    inputs, data_samples, k
                )
                pad_size_dict[k] = [
                    data.metainfo.get("padding_size") for data in data_samples
    ...
    def _preprocess_data_sample(
        self, data_samples: SampleList, training: bool
    ) -> DataSample:
        ...
        for data_sample in data_samples:
            ...
            for key in self.data_keys:
                if not hasattr(data_sample, key):
                    ...
                    print_log(
                        f"Cannot find key '{key}' in data sample.", "current", WARNING
                    )
                    break

                data = data_sample.get(key)
                data_channel_order = self._parse_channel_order(key, data, data_sample)
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if isinstance(_batch_inputs, torch.Tensor):
            _batch_inputs, _batch_data_samples = self._preprocess_image_tensor(
                _batch_inputs, _batch_data_samples
            )
        elif is_seq_of(_batch_inputs, torch.Tensor):
            _batch_inputs, _batch_data_samples = self._preprocess_image_list(
                _batch_inputs, _batch_data_samples
            )
        elif isinstance(_batch_inputs, dict):
            _batch_inputs, _batch_data_samples = self._preprocess_dict_inputs(
                _batch_inputs, _batch_data_samples
            )
            ...
            raise ValueError(
                "Only support following inputs types: "
    ...
    def _destruct_norm_and_conversion(
        self,
        batch_tensor: Tensor,
        ...
        if isinstance(data_samples, list):
            ...
            inputs_order = self._parse_channel_order(
                output_key, batch_tensor, data_samples
            )
    ...
    def _destruct_padding(
        ...
        if isinstance(data_samples, list):
            ...
            if "padding_size" in data_samples[0].metainfo_keys():
                pad_infos = [sample.metainfo["padding_size"] for sample in data_samples]
            else:
                pad_infos = None
        else:
        ...
        if pad_infos is None:
            if self._done_padding:
                print_log(
                    ...
                    "you have called 'self.forward' properly.",
                    "current",
                    WARNING,
                )
        ...
        if same_padding:
            ...
            for idx, pad_info in enumerate(pad_infos):
                ...
                unpadded_tensor = batch_tensor[idx][..., : h - padded_h, : w - padded_w]
