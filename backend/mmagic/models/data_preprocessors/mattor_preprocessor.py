...
class MattorPreprocessor(DataPreprocessor):
    ...
    def _proc_batch_trimap(self, batch_trimaps: torch.Tensor):
        ...
        if self.proc_trimap == "rescale_to_zero_one":
            ...
            raise ValueError(f"proc_trimap = {self.proc_trimap} is not supported.")

        return batch_trimaps

    def _preprocess_data_sample(self, data_samples: SampleList, training: bool) -> list:
            ...
            for key in self.data_keys:
                if not hasattr(data_sample, key):
                    ...
                    if key != "gt_fg" and not training:
                        ...
                        # not print log
                        print_log(
                            f"Cannot find key '{key}' in data sample.",
                            "current",
                            WARNING,
                        )
                    break
                ...
                if training:
                    data = data / 255.0  # NOTE: divided by 255
    ...
    def forward(
        self, data: Sequence[dict], training: bool = False
