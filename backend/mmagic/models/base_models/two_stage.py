...
class TwoStageInpaintor(OneStageInpaintor):
    ...
    def forward_tensor(
        ...
        fake_imgs = stage2_fake_res * masks + masked_imgs * (1.0 - masks)
    ...
    def train_step(self, data: List[dict], optim_wrapper: OptimWrapperDict) -> dict:
        ...
        if self.train_cfg.disc_step > 0:
            ...
            if self.disc_input_with_mask:
                disc_input_x = torch.cat([stage2_fake_img.detach(), mask], dim=1)
            else:
        ...
        stage2_results = dict(fake_res=stage2_fake_res, fake_img=stage2_fake_img)
