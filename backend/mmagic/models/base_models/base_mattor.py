...
ForwardResults = Union[
    Dict[str, torch.Tensor], List[DataSample], Tuple[torch.Tensor], torch.Tensor
]


def _pad(
    batch_image: torch.Tensor, ds_factor: int, mode: str = "reflect"
...
def _interpolate(
    batch_image: torch.Tensor, ds_factor: int, mode: str = "bicubic"
) -> Tuple[torch.Tensor, Tuple[int, int]]:
...
class BaseMattor(BaseModel, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.train_cfg = (
            ConfigDict(train_cfg) if train_cfg is not None else ConfigDict()
        )
        self.test_cfg = ConfigDict(test_cfg) if test_cfg is not None else ConfigDict()
    ...
    def resize_inputs(self, batch_inputs: torch.Tensor) -> torch.Tensor:
        ...
        if resize_method == "pad":
            ...
            batch_images, _ = _interpolate(batch_images, size_divisor, resize_mode)
            batch_trimaps, _ = _interpolate(batch_trimaps, size_divisor, "nearest")
        else:
    ...
    def restore_size(
        self, pred_alpha: torch.Tensor, data_sample: DataSample
    ) -> torch.Tensor:
    ...
    def convert_to_datasample(
        self, predictions: List[DataSample], data_samples: DataSample
    ) -> List[DataSample]:
