...
class BaseGAN(BaseModel, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        if discriminator:
            if isinstance(discriminator, dict):
                ...
                discriminator = MODELS.build(discriminator, default_args=disc_args)
    ...
    @staticmethod
    def gather_log_vars(log_vars_list: List[Dict[str, Tensor]]) -> Dict[str, Tensor]:
        ...
        for k in log_keys:
            assert all(
                [k in log_vars for log_vars in log_vars_list]
    ...
    def _init_ema_model(self, ema_config: dict):
        ...
        src_model = (
            ...
            if is_model_wrapper(self.generator)
            else self.generator
        )
    ...
    def _get_valid_model(self, batch_inputs: ForwardInputs) -> str:
        ...
        assert sample_model in ["ema", "ema/orig", "orig"], (
    ...
    def forward(
        ...
        if sample_model in ["ema", "orig"]:
            ...
            batch_sample_list = gen_sample.split(
                allow_nonseq_value=True,
            )

        else:  # sample model is 'ema/orig
            outputs_orig = self.generator(noise, return_noise=False, **sample_kwargs)
            outputs_ema = self.generator_ema(noise, return_noise=False, **sample_kwargs)
            outputs_orig = self.data_preprocessor.destruct(outputs_orig, data_samples)
            outputs_ema = self.data_preprocessor.destruct(outputs_ema, data_samples)
    ...
    def train_step(
        self, data: dict, optim_wrapper: OptimWrapperDict
    ) -> Dict[str, Tensor]:
