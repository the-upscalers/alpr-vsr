...
FORWARD_RETURN_TYPE = Union[
    dict, torch.Tensor, Tuple[torch.Tensor, torch.Tensor], SampleList
]
...
class OneStageInpaintor(BaseModel):
    ...
    def train_step(self, data: List[dict], optim_wrapper: OptimWrapperDict) -> dict:
        """Train step function.
        ...
        if self.train_cfg.disc_step > 0:
            ...
            disc_losses = self.forward_train_d(fake_img.detach(), False, is_disc=True)
            ...
            if self.with_gp_loss:
                loss_d_gp = self.loss_gp(self.disc, gt_img, fake_img, mask=mask)
        ...
        results, g_losses = self.generator_loss(
            fake_res, fake_img, gt_img, mask, masked_img
        )
    ...
    def forward_train_d(
        self, data_batch: torch.Tensor, is_real: bool, is_disc: bool
    ) -> dict:
    ...
    def generator_loss(
        ...
        if self.with_l1_valid_loss:
            loss_loss_l1_valid = self.loss_l1_valid(fake_res, gt, weight=1.0 - mask)
    ...
    def forward_tensor(
        self, inputs: torch.Tensor, data_samples: SampleList
        ...
        fake_imgs = fake_reses * masks + masked_imgs * (1.0 - masks)
