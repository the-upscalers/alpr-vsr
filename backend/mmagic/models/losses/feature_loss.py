...
class LightCNNFeature(nn.Module):
    ...
    def init_weights(
        self, pretrained: Optional[str] = None, strict: bool = True
    ) -> None:
...
class LightCNNFeatureLoss(nn.Module):
    ...
    def __init__(
        self, pretrained: str, loss_weight: float = 1.0, criterion: str = "l1"
    ) -> None:
        super().__init__()
        self.model = LightCNNFeature()
        if not isinstance(pretrained, str):
            warnings.warn(
                "`LightCNNFeature` model in FeatureLoss " + "should be pretrained"
            )
        ...
        if criterion == "l1":
            ...
            raise ValueError(
                "'criterion' should be 'l1' or 'mse', " f"but got {criterion}"
            )
