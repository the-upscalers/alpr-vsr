...
from ...structures.data_sample import DataSample
from ...utils.typing import ForwardInputs
...
def generation_init_weights(module, init_type="normal", init_gain=0.02):
    ...
    def init_func(m):
        ...
        if hasattr(m, "weight") and (
            ...
            init_info = (
                f"Initialize {m.__class__.__name__} by 'init_type' " f"{init_type}."
            )
            ...
            init_info = (
                f"{m.__class__.__name__} is BatchNorm2d, initialize "
...
def get_valid_noise_size(
    noise_size: Optional[int], generator: Union[Dict, nn.Module]
) -> Optional[int]:
...
def get_valid_num_batches(
    batch_inputs: Optional[ForwardInputs] = None, data_samples: List[DataSample] = None
) -> int:
    ...
    if batch_inputs is not None:
        ...
        num_batches_dict = {
            k: v.shape[0] for k, v in batch_inputs.items() if isinstance(v, Tensor)
        }
    ...
    if not (num_batches_inputs or num_batches_samples):
        print_log(
            ...
            "This may leads to potential error.",
            "current",
            logging.WARNING,
        )
        return 1
...
def build_module(
    module: Union[dict, nn.Module], builder: Registry, *args, **kwargs
) -> Any:
...
def set_xformers(module: nn.Module, prefix: str = "") -> nn.Module:
    ...
    if not xformers_is_enable():
        print_log(
            "Do not support Xformers. Please install Xformers first. "
        ...
        if isinstance_str(module, "BasicTransformerBlock"):
            ...
            if is_mmagic:
                if is_wrapper is None:
                    raise NotImplementedError("Specific ToMe block not implemented")
                elif not is_wrapper:
