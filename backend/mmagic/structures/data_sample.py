...
from ..utils.img_utils import all_to_tensor
...
class DataSample(BaseDataElement):
    ...
    def set_predefined_data(self, data: dict) -> None:
        ...
        metainfo = {
            self.META_KEYS[k]: v for (k, v) in data.items() if k in self.META_KEYS
        }
        self.set_metainfo(metainfo)

        data = {self.DATA_KEYS[k]: v for (k, v) in data.items() if k in self.DATA_KEYS}
    ...
    def stack(cls, data_samples: Sequence["DataSample"]) -> "DataSample":
        ...
        assert all([data.metainfo_keys() == meta_keys for data in data_samples])
    ...
    def split(self, allow_nonseq_value: bool = False) -> Sequence["DataSample"]:
