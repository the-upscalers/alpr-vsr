...
export default function DataForm({ annotationData, setAllAnnotations }) {
	...
	const handleSubmit = (e) => {
		...
		setAllAnnotations((prev) =>
			prev.map((annotation) =>
				annotation.id === data.id ? { ...annotation, ...data } : annotation
			)
		);
	};
			...
			<form onSubmit={handleSubmit}>
				<input
					...
					onChange={(e) =>
						setData((prev) => ({
							...prev,
							data: { ...prev.data, annotationName: e.target.value },
						}))
					}
				...
				<button className="bg-blue-600 text-white px-3 py-1 rounded-md mt-3 w-full hover:bg-blue-700">
					Save
				</button>
			</form>
		</div>
